...
| R5 | effective table repair (header shading/size) | header repair BUILT and applied: on the real report the header row's non-house fill/size/colour is normalised to the house style so the LITable navy+white+bold header renders — but ONLY on cells with no tracked change. Investigation found ~40% of non-house header cells are a reviewer's TRACKED formatting edits (`tcPrChange`); those are PRESERVED and flagged for human accept/reject, never rewritten. Result: 853 stray header cells repaired (failing tables 322→103), preservation clean, 0 rollbacks, valid output. R5 stays OPEN on the tracked/review remainder (never claimed clean). `engine.py _repair_stray_header_formatting` |
